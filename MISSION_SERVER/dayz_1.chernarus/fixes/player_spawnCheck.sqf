...
_tooManyZs = {alive _x} count (_position nearEntities ["zZombie_Base",200]) > dayz_maxGlobalZombies;

{
    if (!_tooManyZs) then {
		_numCleanup=_x getVariable ["despawnedZeds", 0];						////if zeds have despawned, respawn them near their building
		if (_numCleanup > 0) then {												//////////below - get var spawnedzeds - if buiilding didn't spawn zeds, just spawn zeds - if it did, check variable zedSpawnT
			[_x,_numCleanup] call building_REspawnZombies;									/////////////if 30 mins have passed, then spawn zombies again to be changed later
		};
		...
		if (!_shouldNotZeds) then {
			_bPos = getPosATL _x;
			_bWithin = count _nearby;
			_zombiesNum = {alive _x} count (_bPos nearEntities ["zZombie_Base",200]);
			if ((_zombiesNum < 300) and (_zombiesNum < (7 * _bWithin))) then {
				[_x] call building_spawnZombies;
			};
		};
